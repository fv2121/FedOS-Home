...
export type TaskCreateInput = {
  ...
  tags?: string[];
  approved_from_proposed_action_id?: string | null;
};
...
async function ensureUncategorizedCategoryIdInTx(
  tx: Prisma.TransactionClient,
): Promise<string> {
  const existing = await tx.category.findUnique({ where: { slug: "uncategorized" } });
...
export async function createTask(input: TaskCreateInput, actor: ActorType = "user") {
  return prisma.$transaction((tx) => createTaskInTx(tx, input, actor));
}

/**
 * Transaction-aware task creation. Use this when callers need to wrap task
 * creation in a larger transaction (e.g. proposal approval).
 */
export async function createTaskInTx(
  ...
  const categoryId = input.category_id ?? (await ensureUncategorizedCategoryIdInTx(tx));
  ...
  const task = await tx.task.create({
    data: {
      ...
      approved_from_proposed_action_id: input.approved_from_proposed_action_id ?? null,
    },
    include: TASK_INCLUDE,
  });
